...
export async function analyzeProject({project,profile,gates,rootDir,previous=null}){
  ...
  const response=await getHeaders(project.url);let desktop=null,mobile=null,scanError=null;
  try{[desktop,mobile]=await Promise.all([scanViewport(project.url,{width:1440,height:1000,screenshotPath:desktopPath}),scanViewport(project.url,{width:390,height:844,mobile:true,screenshotPath:mobilePath})]);}catch(error){scanError=error.message;}
  const origin=(()=>{try{return new URL(project.url).origin}catch{return project.url}})();const dom=desktop?.dom||mobile?.dom||{};const linkCheck=await brokenLinkRatio(dom.internalLinks);const [robots,sitemap]=await Promise.all([checkEndpoint(origin,'/robots.txt'),checkEndpoint(origin,'/sitemap.xml')]);
  ...
  const scan={url:response.finalUrl||project.url,metrics,dom:combinedDom,headers:headerSignals(response.headers),checks,console:{errorCount:(desktop?.console?.errorCount||0)+(mobile?.console?.errorCount||0),errors:[...(desktop?.console?.errors||[]),...(mobile?.console?.errors||[])].slice(0,12)},network:{failedRequestCount:(desktop?.network?.failedRequestCount||0)+(mobile?.network?.failedRequestCount||0),failures:[...(desktop?.network?.failures||[]),...(mobile?.network?.failures||[])].slice(0,12)},scanError};
  const automatedScores=deriveAutomatedScores(scan,profile);
  let aiReview=null;
  try{
    aiReview=await reviewWithAI({project,profile,desktopScreenshot:desktop?desktopPath:null,mobileScreenshot:mobile?mobilePath:null,automatedScores,scan,rootDir});
  }catch(e){
    aiReview={error:e.message,provider:process.env.ARCHIC_AI_PROVIDER||'auto'};
  }
  ...
  return{id:project.id,name:project.name,url:project.url,finalUrl:scan.url,repository:project.repository||null,profile:project.profile,profileLabel:profile.label,positioning:project.positioning,market:project.market,primaryGoal:project.primaryGoal,status:response.httpOk?(scanError?'partial':'ok'):'critical',score,rawScore:Number(rawScore.toFixed(1)),cap:gated.cap,tier:qualityTier(score),delta:scoreDelta(score,previous?.score),categoryScores:Object.fromEntries(Object.entries(categoryScores).map(([k,v])=>[k,Number(v.toFixed(1))])),gates:gated.active,issues,topPriority:issues[0]||null,aiReview:aiReview?{provider:aiReview.provider||null,model:aiReview.model||null,summary:aiReview.summary||null,strengths:aiReview.strengths||[],confidence:aiReview.confidence??null,error:aiReview.error||null,fallbackFrom:aiReview.fallbackFrom||null,fallbackReason:aiReview.fallbackReason||null}:null,metrics:{lcpMs:metrics.lcpMs??null,mobileLcpMs:metrics.mobileLcpMs??null,cls:metrics.cls??null,ttfbMs:metrics.ttfbMs??null,totalBlockingMs:metrics.totalBlockingMs??null,transferKb:metrics.transferKb??null,resourceCount:metrics.resourceCount??null},checks:{httpOk:checks.httpOk,status:checks.status,horizontalOverflow:checks.horizontalOverflow,robots:checks.robots,sitemap:checks.sitemap,brokenInternalLinks:linkCheck.broken.length,consoleErrors:scan.console.errorCount,failedRequests:scan.network.failedRequestCount},screenshots:desktop?{desktop:`/screenshots/${project.id}/desktop.jpg`,mobile:`/screenshots/${project.id}/mobile.jpg`}:null,reviewedAt:new Date().toISOString(),durationMs:Date.now()-startedAt.getTime()};
}
